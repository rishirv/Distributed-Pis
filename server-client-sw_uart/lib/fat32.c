...
fat32_fs_t fat32_mk(mbr_partition_ent_t *partition) {
  ...
  fat32_volume_id_print("Boot sector", boot);
  ...
  struct fsinfo *fsinfo = pi_sec_read(partition->lba_start + 1, 1);
  fat32_fsinfo_check(fsinfo);
  fat32_fsinfo_print("FS info sector", fsinfo);
  ...
  unsigned n_entries = boot_sector.nsec_per_fat * (boot_sector.bytes_per_sec / 32); // from the boot sector
...
static uint32_t get_cluster_chain_length(fat32_fs_t *fs, uint32_t start_cluster) {
  ...
  while (fat32_fat_entry_type(cluster_num) == LAST_CLUSTER) {
    count++;
...
// Helper function.
static void read_cluster_chain(fat32_fs_t *fs, uint32_t start_cluster, char *data) {
  // TODO: Walk the cluster chain in the FAT until you see a cluster where
  ...
  while(fat32_fat_entry_type(cluster_num) != LAST_CLUSTER) {
    uint32_t cluster_lba = cluster_to_lba(fs, cluster_num);
...
static fat32_dirent_t *get_dirents(fat32_fs_t *fs, uint32_t cluster_start, uint32_t *dir_n) {
  ...
  uint32_t n_bytes = n_clusters * fs->sectors_per_cluster * boot_sector.bytes_per_sec;
  // TODO: read in the whole directory (see `read_cluster_chain`)
  char *data = kmalloc(n_bytes);
  read_cluster_chain(fs, cluster_start, data);

  *dir_n = n_bytes / sizeof(fat32_dirent_t);
...
pi_directory_t fat32_readdir(fat32_fs_t *fs, pi_dirent_t *dirent) {
  ...
  uint32_t n_dirents;
  fat32_dirent_t *d = get_dirents(fs, dirent->cluster_id, &n_dirents);
  ...
  // `dirent_convert`.
  uint32_t pi_dirent_index = 0;
  for (int i = 0; i < n_dirents; i++) {
  
    if (fat32_dirent_free(&d[i])) {
      continue;
    } else if (fat32_dirent_is_lfn(&d[i])) {
      continue;
    } else if (d[i].attr == FAT32_VOLUME_LABEL) {
      continue;
    }
    pi_dirents[pi_dirent_index] = dirent_convert(&d[i]);
    pi_dirent_index++;
  }
...
pi_dirent_t *fat32_stat(fat32_fs_t *fs, pi_dirent_t *directory, char *filename) {
  ...
  for (int i = 0; i < n_dirents; i++) {
    ...
    if (fat32_dirent_free(d)) {
      ...
      continue;
    } else if (dirents[i].attr == FAT32_VOLUME_LABEL) {
      continue;
    }
    char *name = kmalloc(12);
    fat32_dirent_name(d, name);
    if (strcmp(filename, name) == 0) {
      *pi_dirent = dirent_convert(d);
      return pi_dirent;
    }
  }
...
pi_file_t *fat32_read(fat32_fs_t *fs, pi_dirent_t *directory, char *filename) {
  ...
  uint32_t cluster_length = get_cluster_chain_length(fs, dirent->cluster_id);

  // TODO: allocate a buffer large enough to hold the whole file
  uint32_t n_bytes = cluster_length * boot_sector.sec_per_cluster * boot_sector.bytes_per_sec;
  char *buf = kmalloc(n_bytes);
