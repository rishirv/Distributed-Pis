...
int sw_uart_gets(sw_uart_t *uart, const char *start_seq, const char *stop_seq, char *buf, size_t buflen) {
    size_t start_len = strlen(start_seq);
    ...
    while (1) {
        c = sw_uart_get8(uart);
        if (c == -1) {
            return -1; // Error: timed out or disconnected
        }
        if (c == start_seq[i]) {
            i++;
            if (i == start_len) {
                break; // Start sequence found
            }
        } else {
            i = 0; // Reset index
        }
    ...
    // Read data until stop sequence or buflen is reached
    i = 0;
    while (i < buflen - 1) {
        c = sw_uart_get8(uart);
        if (c == -1) {
        ...
        if (c == stop_seq[0] && i >= stop_len - 1 && memcmp(&buf[i - stop_len + 1], stop_seq, stop_len) == 0) {
            break; // Stop sequence found
        }
        buf[i++] = c;
    }

    // Null-terminate buffer and return number of characters read
    buf[i] = '\0';
    return i;
}
