...
void setup() {
  ...
  uint8_t ipLSB = WiFi.softAPIP()[3];  
  Serial.printf("Starting server with IP: %s, LS 4 bits: %x\n", WiFi.softAPIP().toString().c_str(), ipLSB & 0xf);
...
void loop() {
  ...
  if(client){
    Serial.print("Found client with IP: "); 
...
#include <ESP8266mDNS.h>
//server configured to listen on port 1001
WiFiServer server(80); // changed from 80
WiFiClient client;
...
MDNS.addService("poop", "lab", 80); // Announce service on port 1001
...
if(clientLocal) {
  Serial.println("added client");
  Serial.printf("Found client with IP: %x", clientLocal.remoteIP().toString());
  clientLocal.write("argh");
  //client = clientLocal;
}
if(client){
  //Serial.printf("Found client with IP: %x", client.remoteIP());
  //client.write("argh");
  Serial.println("weirdly found uninitalized client!");
}
